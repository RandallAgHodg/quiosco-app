...
const ResumeProduct = ({ product }) => {
  const { handleUpdateAmmount, handleChangeModal, handleProductDelete } =
    useQuiosco();
  ...
  return (
    <div className="shadow p-5 mb-3 flex gap-10 items-center">
      ...
      <div className="md:w-4/6">
        ...
        <p className="text-3xl font-bold mt-2">Ammount: {product.ammount}</p>
        <p className="text-3xl text-amber-500 font-bold mt-2">
          Price: {formatMoney(product.price)}
        </p>
        <p className="text-lg text-gray-700 mt-2">
          Subtotal: {formatMoney(product.price * product.ammount)}
        </p>
      </div>
      ...
      <div>
        <button
          type="button"
          onClick={() => {
            handleUpdateAmmount(product.id);
            handleChangeModal();
          }}
